...
<?php if(!get_theme_mod('sq_disable_sidebar') ) :   ?>
	<main class="sq-content">	
<?php endif;  ?>
		
		<?php if(get_theme_mod('sq_blog_style') == 'grid' || get_theme_mod('sq_blog_style') == 'full_width_then_grid') : ?>
		...
		<?php if ( have_posts() ) : ?>
			...
			<?php if(get_theme_mod('sq_blog_style') == 'full_width_then_grid') : 
						if( $wp_query->current_post == 0 && !is_paged() ) :
							get_template_part('content/loop/post', 'regular');
						else :
							get_template_part('content/loop/post', 'grid');
						endif;
					elseif(get_theme_mod('sq_blog_style') == 'list') : 
							get_template_part( 'content/loop/post', 'list' );
					elseif(get_theme_mod('sq_blog_style') == 'list-circle') : 
							get_template_part( 'content/loop/post-list', 'circle' );							 			
					elseif(get_theme_mod('sq_blog_style') == 'grid') : 
							get_template_part( 'content/loop/post', 'grid' ); 
					else :
							get_template_part( 'content/loop/post', 'regular' );
					endif; ?>
